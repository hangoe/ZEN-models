...
normalisation) fig1/fig2/fig3a/fig3b lived in data/outputs/figures/
mga_tests/archive_v9_0_capex_periods_batch4_minmax/, deleted outright by
user request along with this project's other archived MGA-test batches
(git history has them if needed) -- that run's own crosseffects figure
(fig4a/b, from the now-deleted plot_mga_periods_regional_capex.py) was
deleted outright rather than archived even earlier, once
...
mga_capex_periods_common.py (still used by plot_mga_cum_regional_capex.py
and plot_mga_regional_investment_whatif.py). axis_value()
...
fig3a/3b (this script's own max_separation/ci_lower method-comparison pair,
one row: each supf-mode run's own live, evolving approximation,
reference-equivalent to near_optimal_tools' docs/examples/
method_comparison.ipynb) used to live here too. Removed once
plot_mga_ts_resolution.py's own ts-resolution convergence figures (fig4a/4b
there) superseded it and were renamed fig3a/3b in turn -- see that script's
module docstring, and load_native_ci_history/_cum_seconds_wallclock/
_calibrate_cum_seconds/REAL_ELAPSED_SECONDS below, which it imports directly
...
# ── Shared timing helpers for plot_mga_ts_resolution.py's fig3a/3b ─────────
# This script no longer generates fig3a/3b itself (an earlier
# max_separation/ci_lower method-comparison pair, matching near_optimal_
# tools' docs/examples/method_comparison.ipynb, lived here and was removed
# once plot_mga_ts_resolution.py's own convergence-vs-ts-resolution fig4a/4b
# superseded it and was renamed to fig3a/3b -- see that script's module
# docstring). What remains here is load_native_ci_history and the wall-clock
# accounting it depends on (_cum_seconds_wallclock, _calibrate_cum_seconds,
# REAL_ELAPSED_SECONDS), which plot_mga_ts_resolution.py imports directly
# rather than duplicating: ci_lower (fraction_well_explored's CI lower
# bound) does not need solving here at all -- every supf/batch-mode run
# already evaluates this exact metric against its own live approximation
# once per iteration during the run itself (ci_convergence_metric/
# batch_oracle's own convergence check) and logs the result straight into
# its diagnostics.csv, so it is read directly off disk at full
# per-iteration density rather than resampled. "seconds" started out as just
# the cumulative ZEN-garden solving_time from each solve's benchmarking.json
# (deliberately not wall-clock around the whole loop like the reference
# notebook's TimedCallback), but that alone turned out to be wrong in two
# separate ways this project verified directly against Euler's own SLURM
# job accounting -- see _cum_seconds_wallclock (BATCH_MODES' concurrent
# solves need a max, not a sum, per outer iteration) and
...
def _cum_seconds_wallclock(points: list[tuple[str, np.ndarray, float]],
    ...
    as the mode's own X/A/b) -- the array load_native_ci_history indexes
    into for fig3b's x-axis (plot_mga_ts_resolution.py).
...
# own real elapsed time on fig3b (plot_mga_ts_resolution.py) until an entry
# is added.
