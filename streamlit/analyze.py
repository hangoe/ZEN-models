...
def fig_carrier_energy_all(
    ...
    """One row per carrier per metric (production, then consumption), A|B each."""
    n = len(INDUSTRY_HEAT_CARRIERS_ENERGY)
    rows = n * 2
    fig, axes = plt.subplots(rows, 2, figsize=(15, 6.5 * rows))
    ...
    for i, carrier in enumerate(INDUSTRY_HEAT_CARRIERS_ENERGY):
        ...
        plot_stacked_bars_years_pair(axes[row_prod, 0], axes[row_prod, 1], prod_a, prod_b,
                                     f"{label} — Production", "GWh", name_a, name_b)
        plot_stacked_bars_years_pair(axes[row_cons, 0], axes[row_cons, 1], cons_a, cons_b,
...
def fig_carrier_products_production(
    ...
    """One row per product (glass, ceramic, paper, food): annual production, A|B each."""
    n = len(INDUSTRY_HEAT_CARRIERS_PRODUCT)
    # Half-height rows (3.25 * n instead of 6.5 * n) so product plots stay compact.
    fig, axes = plt.subplots(n, 2, figsize=(15, 3.25 * n))
...
def fig_boiler_hp_production(
    ...
    """One row per temperature level: boiler & HP output (A|B), with NG price overlay."""
    ng_price_a = get_import_price_eur_per_mwh(r_a, "natural_gas")
    ng_price_b = get_import_price_eur_per_mwh(r_b, "natural_gas")
    n = len(INDUSTRY_HEAT_CARRIERS_ENERGY)
    fig, axes = plt.subplots(n, 2, figsize=(15, 6.5 * n))
    ...
    for row, carrier in enumerate(INDUSTRY_HEAT_CARRIERS_ENERGY):
        ...
        add_price_line(axes[row, 0], ng_price_a, "NG import [EUR/MWh]", "#8b0000")
        add_price_line(axes[row, 1], ng_price_b, "NG import [EUR/MWh]", "#8b0000")
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    return fig
...
def fig_capacity_heat_supply(
    ...
    """4 rows × 2 cols: capacity addition & total for boilers+HPs and temp-conversion, A|B each."""
...
def fig_capacity_production(
    ...
    """2 rows × 2 cols: capacity addition and total for production techs, A|B each."""
...
def fig_tes_capacity_addition(
    ...
    """2 rows × 2 cols: TES energy and power capacity addition, A|B each."""
    fig, axes = plt.subplots(2, 2, figsize=(15, 13))
...
def fig_tes_charge_discharge(
    ...
    """2 rows × 2 cols: TES charge and discharge, A|B each."""
    fig, axes = plt.subplots(2, 2, figsize=(15, 13))
...
def fig_heat_demand_by_sector(
    ...
    stacked by heat-carrier temperature level, Model A on the left and Model B
    on the right. The user picks the year via a dropdown in the app.
    """
    ...
    fig.suptitle(f"Industry Sectors — Heat Input by Temperature Level ({year})",
                 fontsize=13, fontweight="bold")
    plot_stacked_bars_years_pair(
        axes[0, 0], axes[0, 1], df_a, df_b,
        "Heat Input by Sector", "GWh", name_a, name_b,
    )
    fig.tight_layout(rect=[0, 0, 1, 0.94])
    return fig
...
def fig_dsm_capacity_addition(
    ...
    """2 rows × 2 cols: DSM energy and power capacity addition, A|B each."""
    fig, axes = plt.subplots(2, 2, figsize=(15, 13))
...
def fig_dsm_charge_discharge(
    ...
    """2 rows × 2 cols: DSM charge and discharge, A|B each."""
    fig, axes = plt.subplots(2, 2, figsize=(15, 13))
...
def fig_storage_comparison(
    ...
    """3 rows × 2 cols: capacity addition comparison across TES, battery, and DSM, A|B each."""
    fig, axes = plt.subplots(3, 2, figsize=(15, 19.5))
...
def _plot_electricity_balance(ax: plt.Axes, df: pd.DataFrame, title: str, name: str) -> None:
    if df.empty:
    ...
    ax.set_ylabel("GWh", fontsize=11)
    ax.set_title(f"{title}\n{name}", fontsize=12, fontweight="bold")
    ax.legend(fontsize=9, frameon=False)


def fig_electricity_capacity_production_consumption(
    r_a: Results, r_b: Results, name_a: str, name_b: str
) -> plt.Figure:
    """3 rows × 2 cols: electricity generation capacity, production, consumption, A|B each."""
    techs_a = get_electricity_producing_techs(r_a)
    techs_b = get_electricity_producing_techs(r_b)
    fig, axes = plt.subplots(3, 2, figsize=(15, 19.5))
    fig.suptitle("Electricity System — Capacity, Production & Consumption",
                 fontsize=13, fontweight="bold")
    plot_stacked_bars_years_pair(
        axes[0, 0], axes[0, 1],
        get_capacity(r_a, techs_a, "power"),
        get_capacity(r_b, techs_b, "power"),
        "Generation Capacity", "GW", name_a, name_b,
    )
    plot_stacked_bars_years_pair(
        axes[1, 0], axes[1, 1],
        get_carrier_production(r_a, "electricity"),
        get_carrier_production(r_b, "electricity"),
        "Production (Generation Mix)", "GWh", name_a, name_b,
    )
    plot_stacked_bars_years_pair(
        axes[2, 0], axes[2, 1],
        get_carrier_consumption(r_a, "electricity"),
        get_carrier_consumption(r_b, "electricity"),
        "Consumption by Technology", "GWh", name_a, name_b,
    )
    fig.tight_layout(rect=[0, 0, 1, 0.97])
    return fig


def fig_electricity_balance(
    r_a: Results, r_b: Results, name_a: str, name_b: str
) -> plt.Figure:
    """1 row × 2 cols: per-year electricity net balance (grouped bars), A|B each."""
    fig, axes = plt.subplots(1, 2, figsize=(15, 6.5))
    fig.suptitle("Electricity — Net Balance (Production vs Consumption, Import, Battery)",
                 fontsize=13, fontweight="bold")
    _plot_electricity_balance(axes[0], get_electricity_balance(r_a), "Net Balance", name_a)
    _plot_electricity_balance(axes[1], get_electricity_balance(r_b), "Net Balance", name_b)
...
def _plot_residual_chrono(ax: plt.Axes, comp: pd.DataFrame, load_col: str,
...
def _plot_residual_duration(ax: plt.Axes, comp: pd.DataFrame, load_col: str,
                            title: str, name: str) -> None:
    if comp.empty:
        ax.set_title(f"{title}\n{name}", fontsize=11, fontweight="bold")
        ax.text(0.5, 0.5, "No data", ha="center", va="center", transform=ax.transAxes)
    ...
    ax.axhline(0, color="black", lw=0.6)
    ax.set_xlim(0, HOURS_PER_YEAR)
    ax.set_xlabel("Hours (sorted, descending)", fontsize=10)
    ax.set_ylabel("GW", fontsize=10)
    ax.set_title(f"{title}\n{name}", fontsize=11, fontweight="bold")
...
def fig_residual_load(
    r_a: Results, r_b: Results, name_a: str, name_b: str,
    year: int, mode: str = "total",
) -> plt.Figure:
    """2 rows × 2 cols residual-load figure for one year, Model A | Model B.
    ...
    comp_a = get_residual_load_components(r_a, year)
    comp_b = get_residual_load_components(r_b, year)
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle(f"Residual Load — {load_label}  ·  {year}",
                 fontsize=13, fontweight="bold")
    _plot_residual_chrono(axes[0, 0], comp_a, load_col, "Chronological", name_a)
    _plot_residual_chrono(axes[0, 1], comp_b, load_col, "Chronological", name_b)
    _plot_residual_duration(axes[1, 0], comp_a, load_col, "Duration curve", name_a)
    _plot_residual_duration(axes[1, 1], comp_b, load_col, "Duration curve", name_b)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    return fig
...
def _plot_storage_dispatch(ax: plt.Axes, net: pd.DataFrame, title: str, name: str) -> None:
...
def fig_storage_use(
    ...
    """2 rows × 2 cols storage figure, Model A | Model B.
    ...
    plot_stacked_bars_years_pair(
        axes[0, 0], axes[0, 1],
        get_storage_flows(r_a, BULK_STORAGE_TECHS, "flow_storage_discharge"),
        get_storage_flows(r_b, BULK_STORAGE_TECHS, "flow_storage_discharge"),
        "Annual Energy Discharged by Storage", "GWh", name_a, name_b,
    )
    _plot_storage_dispatch(axes[1, 0], get_storage_net_ts(r_a, ELEC_STORAGE_TECHS, year),
                           "Electricity Storage — Net Power Duration", name_a)
    _plot_storage_dispatch(axes[1, 1], get_storage_net_ts(r_b, ELEC_STORAGE_TECHS, year),
                           "Electricity Storage — Net Power Duration", name_b)
    fig.tight_layout(rect=[0, 0, 1, 0.96])
    return fig
